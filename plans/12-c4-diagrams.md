...
This document contains standard, high-fidelity C4 Architecture Diagrams modeling the Enterprise Marketplace Platform.

---

## Level 1: System Context Diagram

The System Context diagram provides a high-level, business-oriented view of the **Enterprise Marketplace Platform**, showing the human users (Personas) who interact with the system and its integration boundaries with key third-party dependencies.
...
The Container diagram decomposes the **Enterprise Marketplace Platform**, illustrating the physical deployment units (web apps, API microservices, caching layers, message brokers, and databases), the technical choices per container, and how they communicate with one another using custom protocols.
...
The Component diagram models the internal architecture of the `Ordering.API` microservice container, showcasing how Clean Architecture principles, MediatR CQRS pipeline behaviors, and MassTransit State Machine sagas decouple incoming requests from business validations, domain transactions, and asynchronous message consumers.
...
## Data Flow: Order Checkout Saga

The sequence diagram below displays the end-to-end distributed transaction checkout flow. It highlights the YARP BFF session cookie-to-bearer translation, the high-speed Redis operations, the outbox-orchestrated MassTransit Saga state machine steps, and the final SignalR push notification via Redis Pub/Sub backplane.
