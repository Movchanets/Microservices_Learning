# 7.3.2 — Cart SignalStore (State Management)

> **Ref**: [plans/11-frontend-angular.md](../../plans/11-frontend-angular.md) · [angular-best-practices]

## Goal
Implement the NgRx SignalStore for the Cart feature. Because the cart needs to be accessible from anywhere in the application (e.g., header cart icon, product lists, checkout page), this store will be provided at the root level.

## Prerequisites
- Sub-plan 7.3.1 completed (models and service)

## Steps

### Step 1: Create Cart SignalStore
...
## Done When
- [ ] `CartStore` is implemented as a global SignalStore (`providedIn: 'root'`).
- [ ] Computed signals for `totalItems` and `isEmpty` are defined.
- [ ] Methods `loadCart`, `addToCart`, `updateQuantity`, `removeFromCart`, and `checkout` interact with `CartService` and handle optimistic updates / loading states.
- [ ] Cart loads automatically `onInit`.