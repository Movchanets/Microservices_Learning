...
## Goal
Implement the product detail page at route `/catalog/:id`. Fetches a single product from Catalog.API via the BFF, displays full info with image, tags, seller info, and add-to-cart CTA.

## Prerequisites
- Sub-plans 7.2.1–7.2.3 completed
- Product detail route registered in `catalog.routes.ts`
...
```bash
mkdir -p src/web/src/app/features/catalog/product-detail
```

### Step 2: Implement Product Detail component
...
Navigate to `/catalog/<some-id>` and verify the detail page renders.

## Done When
- [ ] `ProductDetailComponent` at route `/catalog/:id`
- [ ] Fetches product via `CatalogService.getProduct()` → `GET /api/catalog/products/:id` (BFF)
- [ ] Back link to catalog with hover animation
- [ ] Full product display: image, category, name, SKU, price (with currency), tags, description
- [ ] Add to Cart button (wired for Phase 7.3)
- [ ] Loading skeleton state
- [ ] Error state with fallback link
- [ ] Meta section: status, created/updated dates
