# 7.6.2 — Admin NgRx SignalStore

> **Ref**: [plans/11-frontend-angular.md](../../plans/11-frontend-angular.md) · [src/web/Design.md](../../src/web/Design.md)

## Goal
Implement NgRx SignalStore for admin panel state management — user list, pending store verifications, and admin stats.
...
File: `src/web/src/app/features/admin/admin.store.ts`
```typescript
import { computed, inject } from '@angular/core';
import { patchState, signalStore, withComputed, withMethods, withState } from '@ngrx/signals';
import { AdminUserService } from './admin-user.service';
import { AdminStoreService } from './admin-store.service';
import { AdminUser, AdminStore, VerifyStoreRequest } from './admin.models';

interface AdminState {
  users: AdminUser[];
  stores: AdminStore[];
  pendingStores: AdminStore[];
  ...
  withMethods((
    ...
    async verifyStore(storeId: string, request: VerifyStoreRequest): Promise<boolean> {
      ...
      try {
        ...
        // Remove from pending, update in stores list
        patchState(store, {
...
## Done When
- [ ] `AdminStore` with users, stores, pendingStores state
- [ ] Computed signals: pendingCount, verifiedStores, users by role
- [ ] Methods: loadUsers, loadStores, loadPendingStores, verifyStore, updateUserRole, deactivateUser
- [ ] Optimistic update for verifyStore (removes from pending list)
- [ ] Lint and build pass
