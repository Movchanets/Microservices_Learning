...
public class ProductSeeder
{
        ...
        ProductModel product, string token, Guid categoryId, Guid storeId, CancellationToken ct)
    {
        ...
        // Check if product exists by primary SKU
        ...
        // Create primary SKU
        var primarySkuId = await CreateSkuAsync(dto!.Id, product.Sku, product.Price, product.Currency, ct);
        ...
        if (product.Variants != null)
        {
            foreach (var variant in product.Variants)
            {
                ...
                var variantSkuId = await CreateSkuAsync(dto.Id, variantSkuCode, variant.Price, product.Currency, ct);
                if (variantSkuId != null)
                {
                    skuIds2[variantSkuCode] = variantSkuId.Value;
                    _logger.LogInformation("  + Variant SKU {SkuCode} ({Name})", variantSkuCode, variant.Name);
                }
            }
        }
        ...
        _logger.LogInformation("Created product: {Name} with {Count} SKUs", product.Name, skuIds2.Count);
        return (dto.Id, skuIds2);
    }

    private async Task<Guid?> CreateSkuAsync(Guid productId, string skuCode, decimal price, string currency, CancellationToken ct)
    {
        var skuRequest = new
        {
            SkuCode = skuCode,
        ...
        var skuResponse = await _client.PostAsJsonAsync($"/api/catalog/products/{productId}/skus", skuRequest, ct);
        ...
        _logger.LogWarning("Failed to add SKU {SkuCode}: {StatusCode} - {Error}", skuCode, skuResponse.StatusCode, skuError);
        return null;
    }
    ...
    public async Task<Guid?> GetStoreIdAsync(string storeName, CancellationToken ct)
    {
        ...
        var error = await response.Content.ReadAsStringAsync(ct);
        _logger.LogWarning("Failed to fetch stores: {StatusCode} - {Error}", response.StatusCode, error);
        return null;
    }
}
